...
1. `check` — прогоняет `ci.yml`: юнит-тесты (`tests/`), штампы `?v=` у
   CSS/JS актуальны (`scripts/stamp_assets.py`), сборка переводов и новостей,
   все внутренние ссылки и ресурсы резолвятся во всех трёх языковых деревьях
   (`scripts/check_links.py`); без этого деплой не пойдёт;
2. `deploy` — сборка переводов (`scripts/build_i18n.py`) и новостей
   (`scripts/build_news.py`), rsync сайта в
...
3. Он же сразу пересобирает страницы новостей прямо в `/var/www/ndu`
   тем же `scripts/build_news.py`, что и CI, — на всех трёх языках. Пост виден
   на сайте за секунды. Рядом с `api.py` деплой кладёт `build_news.py`,
   `build_i18n.py` и `i18n.py`: генератор импортирует последние два, и без них
   локальная пересборка упадёт.
